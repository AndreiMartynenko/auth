	...
	if mmGetRoleEndpoints.GetRoleEndpointsMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.ctx != nil && !minimock.Equal(*mm_want_ptrs.ctx, mm_got.ctx) {
				mmGetRoleEndpoints.t.Errorf("AccessRepositoryMock.GetRoleEndpoints got unexpected parameter ctx, want: %#v, got: %#v%s\n", *mm_want_ptrs.ctx, mm_got.ctx, minimock.Diff(*mm_want_ptrs.ctx, mm_got.ctx))
			}
...
func (m *AccessRepositoryMock) MinimockGetRoleEndpointsInspect() {
	...
	if !m.GetRoleEndpointsMock.invocationsDone() && afterGetRoleEndpointsCounter > 0 {
		m.t.Errorf("Expected %d calls to AccessRepositoryMock.GetRoleEndpoints but found %d calls",
